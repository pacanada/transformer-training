...
from src.models.transformers import Baseline, Transformer
...
y = torch.load(config.pytorch_dir / "y_train_all.pt")[:,-1].to(device)
X_test = torch.load(config.pytorch_dir / "X_test_all.pt")[:1000].to(device)
y_test = torch.load(config.pytorch_dir / "y_test_all.pt")[:1000, -1].to(device)
wanddblogger = WandbLogger(project_name="crypto_baseline", entity_name="pab_lo4", model_params=config.dict())

print(config.dict())

m = Baseline(config).to(device)
